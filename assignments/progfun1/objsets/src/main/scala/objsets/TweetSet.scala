...
abstract class TweetSet {
   ...
   * and be implemented in the subclasses? 
   * this question is asking if the implentation is the same for both Empty and NonEmpty.
   * if implementation is the same, then implement it here
   * otherwise we need a different version for each of Empty and NonEmpty so we need 
   * to defined them separately for each subclass
   * FWIW in the intsets example of lecture 3.1 we implemented them separately in subclasses
   */
...
object GoogleVsApple {
  ...
  // helper method for googleTweets and appleTweets
  // for each word in the list 'words'
  // check to see if that word in contained within 'text'
  // filter those words out. 
  // if the result of this filter is that there are no word (i.e. length ==0)
  // then return false.
    ...
    lazy val googleTweets: TweetSet = 
      TweetReader.allTweets.filter((tw: Tweet)=>
         containsAny(google, tw.text) 
      )
  lazy val appleTweets: TweetSet = 
     TweetReader.allTweets.filter((tw: Tweet)=>
         containsAny(apple, tw.text) 
      )
